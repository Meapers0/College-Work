...
int main()
{
    ...
    cout << "The current budget is: $" << budget << endl;
    //Try to sell 2 apples and update the budget (this will cause me to sell 2 apples I don't have)
...
//Default constructor for Int_Item initializing name to empty string and all numerical values to 0
Int_Item::Int_Item(){
    name = "";
    sale_price = 0;
    purchase_cost = 0;
...
void Int_Item::set_name(string my_name){
...
//             and output confirmation message to the user.
void Int_Item::set_sale_price(double my_price){
    if( my_price >= 0){
...
//             and output confirmation message to the user.
void Int_Item::set_purchase_cost(double my_cost){
    if( my_cost >= 0){
...
double Int_Item::place_order(int num_ordered){
    if (num_ordered < 1) {
...
void Int_Item::receive_all_orders(){
    if (on_order == 0) {
...
// If number you want to sell is greater than number in stock, report an error message instead of performing the sale
double Int_Item::sell_quantity(int num_sold){
    if (num_sold < 1) {
